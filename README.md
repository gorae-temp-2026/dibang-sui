...
각 앱에 `.env.example`이 있다. 복사 후 실제 값을 채운다.

```bash
# 프론트엔드
cp apps/dibang-wedding/.env.example apps/dibang-wedding/.env
...
- `DATABASE_URL` — Go API의 Postgres 연결
- Sui/zkLogin 관련: `VITE_SUI_PACKAGE_ID`, `VITE_GOOGLE_CLIENT_ID`, `ZKLOGIN_MASTER_SECRET`

환경 전환은 `pnpm env:dev` / `pnpm env:prod` / `pnpm env:status`로 한다.
...
# Go API
pnpm dev:api:local      # localhost:8080 (local DB)
pnpm dev:api:dev        # localhost:8081 (dev DB)
```
...
sui move build           # 빌드
sui move test            # 테스트
sui client publish       # testnet 배포 (배포 정보는 .env.testnet.sui)
```

### 빌드

```bash
pnpm build               # 프론트엔드 전체 빌드 (wedding + guest)
pnpm lint                 # ESLint
```
