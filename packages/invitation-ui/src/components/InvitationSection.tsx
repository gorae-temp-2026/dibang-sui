...
export function InvitationSection({ hosts, groomName, brideName, greetingMessage }: InvitationSectionProps) {
  ...
  // 고인(故) 표기 — 이름 앞에 고인 표시를 붙인다(QA 2026-05-29: 국화 아이콘 → 故).
  const displayName = (name: string, deceased?: boolean) =>
    deceased ? <>{t('invitationUi.invitation.deceasedPrefix')}{name}</> : name;
  ...
  return (
    ...
    >
      ...
      <div className="font-serif font-medium text-xl text-navy text-center tracking-[.02em] mb-[18px]">{t('invitationUi.invitation.title')}</div>
      <div className="w-6 h-px bg-soft-sky mx-auto mb-[18px]" />
      <div className="text-center mb-[26px]">
        <p className="font-serif font-normal text-[15px] leading-[1.95] text-ink tracking-[.02em]">{t('invitationUi.invitation.sonLineBefore')}{displayName(hosts.groomFatherName, hosts.groomFatherDeceased)} · {displayName(hosts.groomMotherName, hosts.groomMotherDeceased)}{t('invitationUi.invitation.sonLineMid')}{stripSurname(groomName)}{t('invitationUi.invitation.sonLineAfter')}</p>
        <p className="font-serif font-normal text-[15px] leading-[1.95] text-ink tracking-[.02em]">{t('invitationUi.invitation.daughterLineBefore')}{displayName(hosts.brideFatherName, hosts.brideFatherDeceased)} · {displayName(hosts.brideMotherName, hosts.brideMotherDeceased)}{t('invitationUi.invitation.daughterLineMid')}{stripSurname(brideName)}{t('invitationUi.invitation.daughterLineAfter')}</p>
      </div>
      <p
