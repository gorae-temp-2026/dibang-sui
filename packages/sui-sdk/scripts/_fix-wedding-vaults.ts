...
// 스크립트가 잡았던 ID들 (Wedding 또는 WeddingLounge 혼재)
const rawIds = [
...
async function resolveWeddingId(rawId: string): Promise<string> {
  const obj = await client.getObject({ id: rawId, options: { showContent: true, showType: true } });
  const t = obj.data?.type ?? '';
  if (t.includes('::wedding::Wedding')) return rawId;
  if (t.includes('::wedding::WeddingLounge')) {
    const fields = (obj.data?.content as any)?.fields;
    return fields?.wedding_id ?? rawId;
  }
  return rawId;
}
...
async function main() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const weddingId = await resolveWeddingId(rawIds[i]);
    const wedding = await getWedding(client, weddingId);
    const eventId = wedding?.eventId ?? '';

    // loungeId — rawId가 Lounge였으면 rawId, 아니면 별도 조회
    let loungeId = rawIds[i] !== weddingId ? rawIds[i] : '';

    console.log(`W${i+1} ${host.name}: wedding=${weddingId.slice(0,15)}... event=${eventId.slice(0,15)}...`);

    // Vault 있는지 확인
    if (wedding?.vaultId) {
      console.log(`  Vault already exists: ${wedding.vaultId.slice(0,15)}...`);
    ...
    try {
      ...
      console.log(`  Vault created: ${vaultId.slice(0,15)}...`);
      result.push({ name: host.name, weddingId, eventId, loungeId, vaultId });
    } catch (e) {
  ...
  console.log('저장: scripts/.e2e-weddings.json');
}
