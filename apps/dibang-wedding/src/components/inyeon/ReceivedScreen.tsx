...
export function ReceivedScreen({ pool, incoming, sentIds, unlockedIds, onAccept, onDecline, onOpenProfile }: ReceivedScreenProps) {
  ...
  return (
    <div className="h-full overflow-y-auto px-4 pb-6 pt-4">
      <div className="mb-3 text-[17px] font-extrabold text-white">{t('inyeon.received.title')}</div>

      {/* 세그 */}
      <div className="mb-4 flex gap-1.5">
        <SegBtn active={seg === 'ieum'} onClick={() => setSeg('ieum')} icon={<Inbox className="h-3.5 w-3.5" />} label={t('inyeon.received.segIeum')} n={incoming.length + sentIds.length} />
        <SegBtn active={seg === 'gwansim'} onClick={() => setSeg('gwansim')} icon={<Eye className="h-3.5 w-3.5" />} label={t('inyeon.received.segInterest')} n={seen.length} />
      </div>

      {seg === 'ieum' ? (
        <>
          <SecHead label={t('inyeon.received.incomingHead')} sub={t('inyeon.received.incomingSub')} />
          {incoming.length ? (
                  ...
                  <div key={r.moiId} className="flex gap-3 rounded-2xl border border-white/8 bg-white/[0.04] p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex gap-2">
                        <button type="button" onClick={() => onDecline(r.moiId)} className="flex-1 rounded-lg border border-white/15 py-2 text-[12px] font-bold text-white/60">{t('inyeon.received.decline')}</button>
                        <button type="button" onClick={() => onAccept(r.moiId)} className="flex-[1.4] rounded-lg bg-gradient-to-br from-[#2E5E8A] to-[#5AA3D6] py-2 text-[12px] font-extrabold text-white">{t('inyeon.received.accept')}</button>
                      </div>
                    </div>
                  </div>
                )
            ...
            <Blank>{t('inyeon.received.incomingEmpty')}</Blank>
          )}

          <SecHead label={t('inyeon.received.sentHead')} sub={t('inyeon.received.sentSub')} className="mt-5" />
          {sentIds.length ? (
            <div className="space-y-2.5">
              {sentIds.map((id) => {
                const m = moiById(id)
                ...
                return (
                  <div key={id} className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.03] p-3">
                    ...
                    <span className="rounded-full bg-[#F8C57A]/15 px-2.5 py-1 text-[10.5px] font-bold text-[#F8C57A]">{t('inyeon.received.waiting')}</span>
                  </div>
                )
              })}
            </div>
          ) : (
            <Blank>{t('inyeon.received.sentEmpty')}</Blank>
          )}
        </>
      ) : (
        <>
          <SecHead label={t('inyeon.received.viewedMeHead')} sub={t('inyeon.received.viewedMeSub')} />
          <Blank>{t('inyeon.received.viewedMeEmpty')}</Blank>

          <SecHead label={t('inyeon.received.viewedHead')} sub={t('inyeon.received.viewedSub')} className="mt-5" />
          {seen.length ? (
            ...
            <Blank>{t('inyeon.received.viewedEmpty')}</Blank>
          )}
        </>
      )}
    </div>
