...
/** 인연 소개글(bio, 인연 전용) — 프로필 '소개글' 섹션. 이음 전에도 공개(정보공개테이블 ①). */
export const MOI_INTRO: Record<number, string> = {
  201: '느긋하게 대화 나누는 걸 좋아해요. 좋은 인연이면 오래 보고 싶어요.',
  202: '주말엔 동네 산책하거나 전시 보러 다녀요. 천천히 알아가요 :)',
  203: '새로운 사람 만나는 게 즐거워요. 편하게 인사 나눠요.',
  204: '운동하고 맛집 찾아다니는 걸 좋아하는 평범한 사람이에요.',
  205: '음악이랑 러닝을 좋아해요. 취향 비슷하면 반가울 것 같아요.',
  206: '여행 다니는 것도, 사진 찍는 것도 좋아해요.',
...
/** 대화방 진입 시 시드 메시지(시스템 안내 2). 방마다 새 배열로 초기화한다.
 *  실제 송수신 메시지는 온체인 노트(useNotes)로 흐르고 DmRoom이 그 노트를 렌더한다 →
 *  여기엔 가짜 상대 인사를 넣지 않는다. (D3) */
