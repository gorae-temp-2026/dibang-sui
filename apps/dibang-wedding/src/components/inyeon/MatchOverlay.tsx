...
export function MatchOverlay({ open, moi, pending, onDismiss, onOpenChat }: MatchOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] mx-auto flex max-w-[420px] flex-col items-center justify-center bg-[radial-gradient(circle_at_50%_36%,rgba(30,58,95,0.95),rgba(11,23,34,0.97))] px-8 text-center">
      <div className="text-[12px] font-extrabold tracking-[0.24em] text-[#F8C57A]">{pending ? t('inyeon.match.pendingTag') : t('inyeon.match.connectedTag')}</div>
      <div className="mb-1 mt-2 text-[33px] font-black text-white">{pending ? t('inyeon.match.pendingTitle') : t('inyeon.match.connectedTitle', { name: moi.name })}</div>
      <p className="max-w-[290px] text-[12.5px] leading-relaxed text-white/80">
        {pending ? t('inyeon.match.pendingDesc') : t('inyeon.match.connectedDesc')}
      </p>
      ...
      {mutualCount > 0 && (
        <div className="mb-4 max-w-[300px]">
          ...
          <div className="mt-1.5">
            <span className="rounded-full bg-white/12 px-2.5 py-1 text-[11.5px] font-bold text-white">{t('inyeon.match.mutualCount', { n: mutualCount })}</span>
          </div>
        </div>
      )}
      ...
      <div className="max-w-[310px] rounded-2xl border border-[#F8C57A]/40 bg-white/[0.07] px-4 py-3">
        <div className="flex items-center justify-center gap-1.5 text-[11.5px] font-extrabold text-[#F8C57A]">
          💎 {t('inyeon.match.creditTitle')}
        </div>
        <div className="mt-1.5 text-[11px] leading-relaxed text-white/75">
          {t('inyeon.match.creditDescPre')}<b className="text-white/90">Moi Credit</b>{t('inyeon.match.creditDescPost')}
        </div>
      </div>
