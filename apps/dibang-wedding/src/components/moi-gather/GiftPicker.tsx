// 선물 고르기 시트 — 채팅 '선물' 버튼에서 진입(인연·라운지 공통). 선물하기(샵) + 받은선물(수신함).
// 선물 = 요네로 구매→상대에게 즉시 전송 → 채팅 선물 카드 + 신뢰 신호(증여=EM·CS). 받은선물 = 광장 꾸미기 사용.
...
export function GiftPicker(props: GiftPickerProps) {
  const { open, onOpenChange, toName, yone, received, pendingItemId, error } = props
  const [tab, setTab] = useState<'send' | 'received'>('send')
  const sending = pendingItemId != null
  const inv = received.map((id) => ITEM_BY_ID[id]).filter(Boolean)
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[88vh]">
        ...
        <div className="mb-3 flex gap-1.5">
          ...
          <button type="button" onClick={() => setTab('received')} className={cn('rounded-full px-3.5 py-1.5 text-[12px] font-bold transition-colors', tab === 'received' ? 'bg-[#1E3A5F] text-white' : 'bg-white/[0.05] text-white/55')}>받은 선물 <span className="rounded-full bg-white/10 px-1.5 text-[10px]">{inv.length}</span></button>
        </div>

        {tab === 'send' ? (
          <div className="grid grid-cols-2 gap-2.5 pb-2">
            {SHOP.map((it) => {
              const afford = yone >= it.yone
              return (
                <div key={it.id} className="flex flex-col overflow-hidden rounded-2xl border border-white/8 bg-white/[0.04]">
                  ...
                  <div className="flex flex-1 flex-col p-2.5">
                    ...
                    <button
                      ...
                      className={cn('mt-2 flex w-full items-center justify-center gap-1 rounded-lg py-2 text-[12px] font-extrabold transition-colors', afford && !sending ? 'bg-gradient-to-br from-[#F8C57A] to-[#E8A865] text-[#5a3a12]' : 'bg-white/[0.06] text-white/35')}
                    >
                      {pendingItemId === it.id ? '보내는 중…' : (<><Coins className="h-3.5 w-3.5" /> {it.yone} 선물</>)}
                    </button>
                  </div>
                </div>
              )
            ...
            {inv.map((it) => (
              <div key={it!.id} className="flex flex-col overflow-hidden rounded-2xl border border-white/8 bg-white/[0.04]">
                <div className="relative flex h-24 items-center justify-center bg-[#f4f1ea]">
                  <img src={it!.url} alt={it!.name} className="h-[84px] w-[84px] object-contain" draggable={false} />
                  <span className="absolute left-2 top-2 rounded-full bg-[#46d77f]/90 px-1.5 py-0.5 text-[8.5px] font-extrabold text-[#0a2414]">받음</span>
                </div>
                <div className="flex flex-1 flex-col p-2.5">
                  <div className="truncate text-[12.5px] font-bold text-white">{it!.name}</div>
