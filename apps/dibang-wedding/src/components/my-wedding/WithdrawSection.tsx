 ...
 */
export function WithdrawSection({ weddingId, vaultId }: { weddingId?: string | null; vaultId?: string | null }) {
  const { address, isAuthenticated } = useZkLogin()
  const { withdraw } = useOnchainHostActions()
  const { data: vault } = useOnchainVault(vaultId ?? undefined)
      ...
      onchainTxMachine.provide({
        actors: {
          submit: fromPromise<string>(async () => {
            if (!vaultId || !weddingId) throw new Error('온체인 금고가 없습니다')
            ...
            return withdraw({ vaultId, capId, amount: mist })
          }),
        },
      }),
    [vaultId, weddingId, address, amount, withdraw],
  )
