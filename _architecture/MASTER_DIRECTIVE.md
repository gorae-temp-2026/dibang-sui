...
- 2026-06-21: **CS authority PageRank 정밀화** — credit.ts CS를 flat in-tie → authority PageRank(PHI-5 정식: 유대받는 쪽 적립 + 고신뢰자에게 받을수록↑, 유대의 질 반영). 부조와 동일 기준선 초과분(비-유대자=0). authority>flat 구별 테스트(hub→x vs z→y). vitest 8/8, tsc 0. → opus 확인. **다음:** 토이 DeFi(#12) / SDK 빌더+guest-web 경계 / 이행(settles) 신호 / 가중치 실데이터 튜닝.
