...
- 2026-06-21: **TS e2e 크레딧 테스트** — credit.test.ts에 현실 다신호 시나리오(웨딩: 초대×2+부조×2+방명록+참석×3, 인연: alice↔bob 매칭) 한 번에 creditFromEvents fold: 부조 서열(guest1>guest2)·혼주 CS>0(방명록·참석 받음)·하객 초대 CS>0·**인연 양쪽 CS>0(Critical1 회귀가드 — 인연→신용 실제 기여)**·전 신용 0~1. Move e2e의 TS 짝 = 통합 결함 양면(컨트랙트 발행 + 신용 fold) 방어. vitest 11/11, tsc 0. **다음:** Wedding 이름 온체인 보유(사용자 결정 대기) / SDK 빌더 정렬+guest-web 경계 / 토이 DeFi(#12) / 통합 리뷰 2회차(다음 묶음 후).
