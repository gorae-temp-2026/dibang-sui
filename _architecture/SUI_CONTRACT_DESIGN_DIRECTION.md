...
> # ⚠️ 레이어 SCOPE — 이 컨트랙트가 *무엇을 하는 곳인가*
> - **이 컨트랙트 = 신뢰 신호 원장 + 신용 substrate.** raw 신호(부조·방명록·초대·선물·참석·매칭)를 온체인에 분류·발행하고, 그 위 신용(전파)을 *계산할 입력*까지가 scope다.
> - **DeFi(무담보 대출 등)·이행축(perf=대여·상환)은 이 레이어가 구현하는 곳이 아니다.** 신용을 *소비하는* 다운스트림 별개 미래 층(결정 #12, "끝단")이다.
> - 그러므로 DeFi/perf를 이 컨트랙트의 **"미구현/안 됨/결함"으로 적지 말 것 — scope 밖이지 gap이 아니다.** (이전 세션이 정확히 이렇게 오분류함.)
> - `settles` 인자·`perf` 무기록치(0.7)는 *미래 DeFi 대비 예비*일 뿐, 현재 scope의 미완이 아니다.
